---
title: "Forecasting Daily Share of Monthly Total with Beta Likelihood"
description: "Recipe: derive a bounded (0,1) target from Peyton Manning pageviews and forecast it with likelihood='beta'."
---
...
Why Beta? Because the target is naturally bounded in (0,1) and often heteroskedastic: variance shrinks near 0 or 1. The `likelihood="beta"` option internally applies a link that guarantees valid predictions.
...
import pandas as pd
y = pd.DataFrame(
    data={"value" : np.sin(np.arange(100)*2*np.pi/30)*0.45 + 0.5},
...
fh = pd.period_range(y.index[-1] + 1, periods=180, freq="D")
...
## 6. Probabilistic forecast (quantiles)
...
ax.plot(y.index.to_timestamp(), y, lw=1, alpha=0.6, color="k")
...
* `noise_scale` controls dispersion of the Beta (smaller => tighter intervals). You can tune it or let the model infer if supported.
