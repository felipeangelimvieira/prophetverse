...
def test_weibull_adstock_parameter_sensitivity():
    ...
    param_sets = [
        {"scale": jnp.array(0.5), "concentration": jnp.array(0.5)},  # Low scale, low concentration
        {"scale": jnp.array(2.0), "concentration": jnp.array(0.5)},  # High scale, low concentration  
        {"scale": jnp.array(0.5), "concentration": jnp.array(2.0)},  # Low scale, high concentration
        {"scale": jnp.array(2.0), "concentration": jnp.array(2.0)},  # High scale, high concentration
    ]
...
def test_weibull_adstock_consistency_across_calls():
    ...
    for i in range(1, len(results)):
        assert jnp.allclose(results[0], results[i], atol=1e-10), f"Results differ between calls {0} and {i}"
...
def test_weibull_adstock_multivariate_input():
    ...
    with numpyro.handlers.seed(rng_seed=0), numpyro.handlers.do(data=params):
        ...
        assert result.shape == (3, 2)  # Same shape as input
        assert jnp.all(result >= 0)
