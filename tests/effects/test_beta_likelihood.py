...
from prophetverse.sktime.univariate import ProphetBeta
...
def make_beta_data(n_samples=100, n_series=1):
    ...
    if n_series == 1:
        ...
        index = pd.MultiIndex.from_product(
            [range(n_series), dates],
            names=['series', 'time']
        )
    
    # Generate data between 0 and 1
    y = pd.Series(
        np.random.beta(a=2, b=5, size=n_samples * n_series),
        index=index
    )
    return y
...
def test_prophet_beta_basic():
    ...
    forecaster = ProphetBeta(
        trend=FlatTrend(),
        exogenous_effects=[
            ('seasonality', LinearFourierSeasonality(sp_list=[7], fourier_terms_list=[1], freq='D'), None)
        ],
