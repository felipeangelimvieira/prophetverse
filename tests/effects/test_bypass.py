...
def test_ignore_input_effect_fit_predict_without_x(ignore_input_effect, sample_data):
    """Test BypassEffect with no exogenous data."""
    y, _, _ = sample_data

    # Fit with no X data
    ignore_input_effect.fit(y=y, X=None, scale=1.0)

    # Transform and predict with no X
    data = ignore_input_effect.transform(None, fh=y.index[-3:])
    predicted_effects = {}
...
def test_ignore_input_effect_with_validation_empty_x(
    ...
    ignore_input_effect_with_validation.fit(y=y, X=X_empty, scale=1.0)

    # Transform and predict should work
    data = ignore_input_effect_with_validation.transform(X_empty, fh=y.index[-3:])
    predicted_effects = {}

    with seed(rng_seed=0):
        result = ignore_input_effect_with_validation.predict(
            data=data, predicted_effects=predicted_effects
        )
...
def test_ignore_input_effect_with_validation_none_x(
    ignore_input_effect_with_validation, sample_data
):
    ...
    ignore_input_effect_with_validation.fit(y=y, X=None, scale=1.0)

    data = ignore_input_effect_with_validation.transform(None, fh=y.index[-3:])
    predicted_effects = {}
