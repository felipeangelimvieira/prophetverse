...
def test_latent_effect_basic():
    ...
    forecaster = Prophetverse(
        ...
        inference_engine=MAPInferenceEngine(
            optimizer=AdamOptimizer(), num_steps=5, num_samples=1
        ),
    )
    ...
    future_dates = pd.date_range(
        start=y.index[-1] + pd.Timedelta(days=1), periods=3, freq="D"
    )
    ...
    assert (
        len(components.columns) >= 3
    )  # Should have at least trend, seasonality, feature1_effect
    ...
    print(
        "✓ Latent effect test passed: latent effects computed but excluded from final mean"
    )
...
def test_latent_effect_available_to_other_effects():
    ...
    # Create future X data for prediction
    future_dates = pd.date_range(
        start=y.index[-1] + pd.Timedelta(days=1), periods=2, freq="D"
    )
    X_future = pd.DataFrame(
        {
